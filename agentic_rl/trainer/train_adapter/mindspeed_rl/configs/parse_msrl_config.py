...
def _gen_megatron_config(config: Dict[str, Any]) -> MegatronConfig:
    ...
    training_params = {
        ...
        "no_masked_softmax_fusion": True, "attention_softmax_in_fp32": True, "no_gradient_accumulation_fusion": True,
        "use_fused_swiglu": True, "use_fused_rotary_pos_emb": True, "bf16": True, "use_distributed_optimizer": True,
        "tokenizer_type": "PretrainedFromHF", "tokenizer_name_or_path": config.get("tokenizer_name_or_path"),
        "global_batch_size": ms_config.get("global_batch_size", 2), "seq_length": ms_config.get("seq_length", 8192),
        ...
        "norm_epsilon": 0.00001, "dataset_additional_keys": ["labels"],
        "data_path": ms_config.get("data_path"), "split": "100,0,0",
        ...
        "lr": config.get("lr"), "lr_decay_style": "constant", "min_lr": config.get("min_lr"),
        "weight_decay": config.get("weight_decay"), "lr_warmup_fraction": config.get("lr_warmup_fraction"),
        "clip_grad": config.get("clip_grad"), "adam_beta1": 0.9, "adam_beta2": 0.999, "initial_loss_scale": 4096,
