...
if [[ ${node_type} == "master" && ${task_type} == "train" ]]; then
  python agentic_rl/start.py --config-name=one_step_off_16node_qwen3_235b_separate_train 2>&1 | tee ${logs_path}/train_logs_${timestamp}.log
  ray stop
elif [[ ${node_type} == "master" && ${task_type} == "rollout" ]]; then
  python agentic_rl/start.py --config-name=one_step_off_32node_qwen3_235b_separate_rollout 2>&1 | tee ${logs_path}/rollout_logs_${timestamp}.log
  ray stop
fi
