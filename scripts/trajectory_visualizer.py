...
def main(trajectory_file: str = "./trajectories/sample_trajectories/search_trajectories.pt", server_port: int = 12345):
    ...
    def update_step_view(traj_idx: int, step_idx: int, filter_option: str):
        ...
        if task_type == "search":
            ...
            question = metadata.get("question_content",
                                    metadata.get("question",
                                                 metadata.get("problem", "No problem statement found")))
        ...
        perf_text += (f"**Completed:** "
                      f"{'✅ Yes' if (num_steps > 0 and getattr(trajectory.steps[-1], 'done', False)) else '❌ No'}")

        question_text = f"**Question:**\n{question}\n\n**Ground Truth Answer:** `{ground_truth}`"

        if num_steps == 0:
            return (position_text, metadata_text, perf_text, question_text,
                    empty_content, empty_content, empty_content, empty_content, empty_content, empty_content)
        ...
        step_perf_text += (f"**Response Structure:** "
        ...
        if step_action:
            if task_type == "search" and isinstance(step_action, list):
                ...
                actions_text = (f"**Generated "
                                f"{'Code' if task_type == 'code' else 'Solution'}:**\n```\n{step_action}\n```")
            else:
                actions_text = format_tool_call_detailed(step_action) \
                    if isinstance(step_action, dict) else str(step_action)
        ...
        if has_finish_action:
            if predicted_answer:
                if task_type == "search":
                    ...
                    final_answer_text = (f"**{'💻' if task_type == 'code' else '🧮'} "
            ...
            if step_idx == num_steps - 1:
                ...
                final_answer_text += f"**Ground Truth:** `{ground_truth}`\n"
                final_answer_text += (f"**Status:** Step {step_idx + 1}/{num_steps} - "
                                      f"{'No finish action' if task_type == 'search' else 'Continuing...'}")

        return (position_text, metadata_text, perf_text, question_text, thinking_text,
                response_text, step_perf_text, actions_text, outputs_text, final_answer_text)
    ...
    with gr.Blocks(theme=gr.themes.Soft(), css=custom_css, title="Agent Trajectory Visualizer") as interface:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                filter_dropdown = gr.Dropdown(choices=["All Trajectories", "Zero Reward (Failed)",
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                current_pos_display = gr.Textbox(label="Current Position",
                                                 interactive=False,
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                with gr.Accordion("🧠 Agent Thinking", open=True):
                    thinking_output = gr.Textbox(label="Internal Reasoning", lines=6,
                                                 interactive=False, elem_classes=["thinking-box"])
        ...
        all_outputs = [current_pos_display, metadata_output, performance_output, question_output,
