...
parser.add_argument("--local-host", type=str, required=True, help="Local host IP")
...
def get_cmd_stdout(cmd):
    ...
    return subprocess.run(args, capture_output=True,
                          shell=False).stdout.decode('utf-8').strip()
...
global_device_list = [
    device_info
    for device_list in global_device_list
...
for device_info in global_device_list:
    ...
    cnt += 1
if (prefill_device_cnt + decode_device_cnt) > len(global_device_list):
    raise ValueError(
        f"prefill_device_cnt + decode_device_cnt must be less than or equal to number of all devices in cluster"
    )
ranktable = {
    "version":
        "1.2",
    ...
    "decode_device_list":
        global_device_list[prefill_device_cnt:prefill_device_cnt + decode_device_cnt],
    "status":
        "completed"
}
