...
任意一项失败可以通过详情链接查看具体问题。如果是CI自身故障，请[联系committer](https://gitcode.com/Ascend/community/blob/master/MindSeriesSDK/sigs/MindSeriesSDK/sig-info.yaml)，或通过评论“rebuild”尝试重新构建。

# Special Interest Group
## 工作目标和范围
1. 技术聚焦
   围绕基于Agentic多训练后端支持，强化学习环境兼容接口等功能进行深入研究，推动技术发展，解决实际问题。
2. 促进协作
...
## 例会
* 周期：每1个月举行一次例会，可通过[Ascend开源社区](https://meeting.ascend.osinfra.cn/)搜索、查看sig-MindSeriesSDK的会议链接。
* 申报议题：通过[sig-MindSeriesSDK Etherpad链接](https://etherpad.ascend.osinfra.cn/p/sig-MindSeriesSDK)进入共享文档，编辑申报议题。
...
* 会议归档：会议纪要位于[sig-MindSeriesSDK Etherpad链接](https://etherpad.ascend.osinfra.cn/p/sig-MindSeriesSDK)。

## 成员列表
[SIG成员列表](https://gitcode.com/Ascend/community/blob/master/MindSeriesSDK/org-info.yaml)。
